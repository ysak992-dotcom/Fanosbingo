...
CREATE TABLE games (
  ...
  -- text[], NOT uuid[]. Production is `_text` (verified against the live prod
  -- database 2026-08-15), and atomic_claim_bingo declares `v_winner_ids text[]`
  -- and appends `p_player_id::text` to match. Declaring uuid[] here made the
  -- fixture disagree with production in a way that only surfaced when
  -- db/test/game_integrity_test.sql ran the REAL claim function against it:
  --
  --   ERROR: COALESCE could not convert type text[] to uuid[]
  --
  -- The game_tick payout tests had passed against the uuid[] version, because
  -- the fixture's own payout trigger was written to match the fixture rather
  -- than production. Two wrongs agreeing is exactly what a fixture must not do.
  winner_ids text[] DEFAULT '{}',
  winner_prize_each integer DEFAULT 0,
  winners_paid boolean DEFAULT false,
  -- The remainder of production's shape, added after a column-by-column diff
  -- against the LIVE prod database on 2026-08-15. Unused by the tests, present
  -- so the fixture cannot be laxer than what the migrations actually run on.
  allow_late_joins boolean DEFAULT false,
  return_to_lobby_at timestamptz
);
...
CREATE TABLE players (
  ...
  -- Production's remaining columns, from the same 2026-08-15 diff.
  -- disqualified_at is NOT decorative: atomic_claim_bingo writes it when a claim
  -- does not hold, so without it the real function throws mid-claim -- which is
  -- how db/test/game_integrity_test.sql found this gap.
...
RETURNS TRIGGER SECURITY DEFINER SET search_path = public LANGUAGE plpgsql AS $$
DECLARE w uuid; prize integer;  -- uuid loop var over text[], as production does
BEGIN
...
-- ---------------------------------------------------------------------------
-- THE REAL WIN LOGIC, not a stub, because this is the code that decides who
-- gets paid.
--
-- atomic_claim_bingo() was `RETURN '{}'` here, which meant every test touching a
-- claim exercised nothing. db/test/game_integrity_test.sql asks whether a player
-- can manufacture a win -- by marking cells they were not dealt, by claiming a
-- card that is not theirs, by claiming numbers that were never called -- and
-- against a stub every one of those attacks "passes".
--
-- Both bodies are copied verbatim from 20251226111837, which is the migration
-- that defines them last. If they drift there, they drift here and the harness
-- stops testing production; that is the same trade the rest of this file makes,
-- and it is why the fixture is a production-SHAPED schema rather than a
-- convenient one.
-- ---------------------------------------------------------------------------
